...
leaderboard/       online service: leaderboard (Cloudflare Worker + D1 database)
...
`wrong`, `levelUp`, `checkpoint`; finds and gold: `discover`, `coin`; town and battle: `hammer`, `clash`, `twang`, `spell`, `horn`, `alarm`, `crumble`). Add new sounds there rather than in a game. Volume and mute come from
...
A game that shares data between players can add its own routes to the same service
(see AGENTS.md). None does at the moment. Every write is rate-limited per device and per
network, and each costs only a few database rows, so the service stays well inside
